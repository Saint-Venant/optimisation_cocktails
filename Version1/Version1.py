...
class planProduction:
    ...
    def updateClusters(self):
        ...
        for i in range(0, len(self.matProd)):
            for j in range(0, len(self.matProd[i])):
                self.clusters[self.matProd[i][j]].append(self.matBoissons[i][j])
    ...
    def ajouteCommande(self, com):
        ...
        indice = 0
        b = com.getBoisson(indice)
        
        ligneMatProd.append(self.nbClusters)
        ligneMatBoissons.append(b)
        
        nbNouvClusters = 1
        indice += 1
        
        while (indice < com.nbBoissons):
            if (com.idBoisson(indice) == ligneMatBoissons[-1].id):
                ligneMatProd.append(ligneMatProd[-1])
            else:
                ligneMatProd.append(ligneMatProd[-1] + 1)
                nbNouvClusters += 1
            c = com.getBoisson(indice)
            ligneMatBoissons.append(c)
            indice += 1
        
        #concaténation de la matrice de production avec sa nouvelle ligne
        if (self.nbClusters == 0):
            self.nbClusters = nbNouvClusters
            self.matProd.append(ligneMatProd)
            self.matBoissons.append(ligneMatBoissons)
        elif (self.matBoissons[-1][-1].id == ligneMatBoissons[0].id):
    ...
    def copy(self):
        ...
        for i in range(0, len(self.matProd)):
            copie.matProd.append(self.matProd[i].copy())
            copie.matBoissons.append(self.matBoissons[i].copy())
    ...
    def ajouteCommandeUltraOpti(self, com):
        ...
        for i in range(0, len(self.matProd)):
            for j in range(0, len(self.matProd[i])):
                if not(self.matBoissons[i][j].id in correspondance):
                    correspondance[self.matBoissons[i][j].id] = self.matProd[i][j]
